...
def poker_no(mazo):
    ...
    while len(mazo) != 0 and len(pilas) != 0 and bucle != 1: 
        ...
        while pila < len(pilas) and len(mazo) != 0 and bucle != 1:
            bandera = 0
            registro.add(tuple( str(mazo), str(pilas) ))
            contador += 1

            pilas[pila].append(mazo.popleft())
            
            while  bandera != 1 and len(pilas[pila]) >= 3 :
                    if pilas[pila][0] + pilas[pila][1] + pilas[pila][-1] in [10, 20, 30]:
                    ...
                    elif pilas[pila][0] + pilas[pila][-2] + pilas[pila][-1] in [10, 20, 30]:
                        mazo.append(pilas[pila].popleft())
                        temporal = pilas[pila].pop()
                        mazo.append(pilas[pila].pop())
                        mazo.append(temporal)
                        
                    elif pilas[pila][-3] + pilas[pila][-2] + pilas[pila][-1] in [10, 20, 30]:
                        temporal = pilas[pila].pop()
                        ...
                        bandera = 1
            if tuple( str(mazo), str(pilas) ) in registro:
                bucle = 1
            pila += 1
...
def main():
    cartas = list(map(int,stdin.readline().strip().split()))
    mazo = []
    while cartas[0] != 0:
        mazo.extend(cartas)
        cartas = list(map(int,stdin.readline().strip().split()))
        if len(mazo) >= 52:
            poker_no(deque(mazo[:52]))
            mazo = mazo[52:]
main()
#end_time = time.time()
